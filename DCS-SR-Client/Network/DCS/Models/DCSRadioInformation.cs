...
public class DCSRadioInformation
{
    ...
    public override bool Equals(object obj)
    {
        ...
        var compare = (DCSRadioInformation)obj;

        if (!name.Equals(compare.name)) return false;
        if (!RadioInfo.FreqCloseEnough(freq, compare.freq)) return false;
        ...
        if (!RadioInfo.FreqCloseEnough(secFreq, compare.secFreq)) return false;
    ...
    internal DCSRadioInformation Copy()
    {
        //probably can use memberswise clone
        return new DCSRadioInformation
        {
            channel = channel,
            enc = enc,
            encKey = encKey,
