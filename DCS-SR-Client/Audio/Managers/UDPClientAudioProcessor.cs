...
using NLog;
using SharpDX.Text;
using RadioReceivingState = Ciribob.DCS.SimpleRadio.Standalone.Common.Network.RadioReceivingState;
...
public class UDPClientAudioProcessor : IDisposable
{
     ...
     private void RetransmitAudio(UDPVoicePacket udpVoicePacket, List<RadioReceivingPriority> radioReceivingPriorities)
        {
            ...
            var receivingWithRetransmit = radioReceivingPriorities.Where(receivingRadio => 
                ...
                //check global
                && !globalFrequencies.Any(freq => RadioInfo.FreqCloseEnough(receivingRadio.ReceivingRadio.freq, freq))
            ...
            {
                radiosWithRetransmit = radiosWithRetransmit.Where(radio => !RadioInfo.FreqCloseEnough(radio.freq, receivingRadio.Frequency));
            }
    ...
    private List<int> CurrentlyBlockedRadios()
    {
        ...
        var currentRadio =
            ...
            if (currentRadio.modulation == Radio.Modulation.FM 
                || currentRadio.modulation == Radio.Modulation.AM 
                || currentRadio.modulation == Radio.Modulation.MIDS 
                || currentRadio.modulation == Radio.Modulation.HAVEQUICK)
            {
        ...
        if (_clientStateSingleton.DcsPlayerRadioInfo.simultaneousTransmission)
                ...
                for (int i = 1; i < 11; i++)
            {
                var radio = _clientStateSingleton.DcsPlayerRadioInfo.radios[i];
                    if ( (radio.modulation == Radio.Modulation.FM || radio.modulation == Radio.Modulation.AM )&& radio.simul &&
        ...
        private bool HasLineOfSight(UDPVoicePacket udpVoicePacket, out float losLoss)
        {
            ...
            {
                if (udpVoicePacket.Modulations[i] == (int)Radio.Modulation.AM 
        ...
        private bool InRange(string transmissingClientGuid, double frequency, out double signalStrength)
        {
            ...
            if (_clients.TryGetValue(transmissingClientGuid, out transmittingClient))
            {
                ...
                //TODO just store the correct type - not the DCS one so we can compare
                var myLatLng = _clientStateSingleton.PlayerCoaltionLocationMetadata.LngLngPosition;
                var clientLatLng = transmittingClient.LatLngPosition;
                ...
                else
                {
                    ...
                    dist = RadioCalculator.CalculateDistanceHaversine(_clientStateSingleton.PlayerCoaltionLocationMetadata.ToNetworkLatLng(), clientLatLng);
                }
  ...
  private List<DCSRadioInformation> PTTPressed(out int sendingOn, bool voice)
        {
            ...
            if ((radioInfo.intercomHotMic
                ...
                || _intercomPtt)
            {
                if (radioInfo.radios[0].modulation == Radio.Modulation.INTERCOM)
                {
            ...
            if (_ptt || _clientStateSingleton.DcsPlayerRadioInfo.ptt)
            {
                ...
                if (_clientStateSingleton.DcsPlayerRadioInfo.simultaneousTransmission)
                {
                    ...
                    if (currentSelected == 0 && currentlySelectedRadio.modulation == Radio.Modulation.INTERCOM && _clientStateSingleton.DcsPlayerRadioInfo.inAircraft == false)
                    {
                    ...
                    {
                        if (radio != null && radio.simul && radio.modulation != Radio.Modulation.DISABLED
                            && (radio.freq > 100 || radio.modulation == Radio.Modulation.INTERCOM)
                                ...
                                if (udpVoicePacket != null)
                                {
                                        ...
                                        for (var i = 0; i < frequencyCount; i++)
                                    {
                                        ...
                                        var radio = _clientStateSingleton.DcsPlayerRadioInfo.CanHearTransmission(
                                            ...
                                            (Radio.Modulation) udpVoicePacket.Modulations[i],
                                        ...
                                        {
                                            if (
                                                radio.modulation == Radio.Modulation.INTERCOM
                                                || radio.modulation == Radio.Modulation.MIDS // IGNORE LOS and Distance for MIDS - we assume a Link16 Network is in place
                                                || globalFrequency
                                                || (
