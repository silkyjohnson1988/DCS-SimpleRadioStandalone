...
namespace Ciribob.DCS.SimpleRadio.Standalone.Client.Utils
{
    public static class RadioHelper
    {
        public static void ToggleGuard(int radioId)
        {
            ...
            {
                if (radio.freqMode == DCSRadioInformation.FreqMode.OVERLAY || radio.guardFreqMode == DCSRadioInformation.FreqMode.OVERLAY)
                {
                    if (radio.secFreq > 0)
                    {
        ...
        public static bool UpdateRadioFrequency(double frequency, int radioId, bool delta = true, bool inMHz = true)
        {
            ...
            if (radio != null && radioId > 0)
            {
                if (radio.modulation != Radio.Modulation.DISABLED
                    && radio.modulation != Radio.Modulation.INTERCOM
                    && radio.freqMode == DCSRadioInformation.FreqMode.OVERLAY)
                {
                    if (delta)
                    {
        ...
        public static DCSRadioInformation GetRadio(int radio)
        {
        ...
        public static void ToggleEncryption(int radioId)
        {
            ...
            if (radio != null && radioId > 0)
            {
                if (radio.modulation != Radio.Modulation.DISABLED) // disabled
                {
                    //update stuff
                    if (radio.encMode == DCSRadioInformation.EncryptionMode.ENCRYPTION_JUST_OVERLAY)
                    {
                        if (radio.enc)
                        {
                            radio.enc = false;
        ...
        public static void SetEncryptionKey(int radioId, int encKey)
        {
            ...
            {
                if (currentRadio.modulation != Radio.Modulation.DISABLED) // disabled
                {
                    //update stuff
                    if ((currentRadio.encMode ==
                         DCSRadioInformation.EncryptionMode.ENCRYPTION_COCKPIT_TOGGLE_OVERLAY_CODE) ||
                        (currentRadio.encMode == DCSRadioInformation.EncryptionMode.ENCRYPTION_JUST_OVERLAY))
                    {
                        if (encKey > 252)
                            encKey = 252;
                        else if (encKey < 1)
        ...
        public static void SetRadioVolume(float volume, int radioId)
        {
            ...
            if (currentRadio != null
                ...
                && currentRadio.volMode == DCSRadioInformation.VolumeMode.OVERLAY)
            {
        ...
        public static void ToggleRetransmit(int radioId)
        {
            ...
            if (radio != null && radioId > 0)
            {
                if (radio.rtMode == DCSRadioInformation.RetransmitMode.OVERLAY)
                {
